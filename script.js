                ...
                <div id="card" class="card mb-3" style="width: 20rem;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between mt-5 detailPart">
                            <button type="button" class="btn btn-warning recipe" onclick="displayMealDetails(${element.idMeal})">Recipe</button>
...
// DISPLAYS THE FAVOURITES MEALS
async function displayFavMealList() {
    ...
    if (favListArr.length==0) {
        ...
        for (let index = 0; index < favListArr.length; index++) {
            await fetchMealsFromApi(url,favListArr[index]).then(data=>{
                html += `
                <div id="card" class="card mb-3 m-2" style="width: 20rem;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between mt-5 detailPart">
                            <button type="button" class="btn btn-warning recipe" onclick="displayFavMealDetails(${data.meals[0].idMeal})">Recipe</button>
                            <button id="main${data.meals[0].idMeal}" class="btn btn-outline-light" onclick="addRemoveToFavList(${data.meals[0].idMeal})" style="border-radius:50%"><i class="fa fa-trash deleteFavorite" aria-hidden="true"></i>
                            </button>
                        </div>
                    </div>
                </div>
...
// DISPLAYS  MEAL DETAILS OF FAVORITE
...
function addRemoveToFavList(id) {
    ...
    for (let index = 0; index < favListArr.length; index++) {
        if (id==favListArr[index]) {
            containFlag=true;
        }
    }
    if (containFlag) {
        let itemIndex = favListArr.indexOf(id);
        favListArr.splice(itemIndex, 1);
        //alert("your meal removed from your favourites list");
        notificationSetting("Removed from fav");
    } else {
        favListArr.push(id);
        //alert("your meal add your favourites list");
        notificationSetting("Added to fav");
    }
...
// DYNAMIC TOAST MSG
function notificationSetting(msg) {
